...
| File | Purpose |
|---|---|
| `PROJECT.md` | Canonical project description — architecture, scene structure, key systems. Update when systems are added, removed, or significantly changed. Imported by root `CLAUDE.md`. |
| `GOAL.md` | Goal of the current session. Overwrite at session start with the new objective and open decisions. Imported by root `CLAUDE.md`. |
...
Each subagent inherits: `PROJECT.md`, the `.AI/` files already produced by earlier steps, and all `.claude/rules/`.
