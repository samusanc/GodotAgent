...
<!-- Always-loaded. No `paths` frontmatter = applies to every file in the project. -->

These rules apply to every file in the project regardless of language or type.

## File Size

- **Hard limit: 50 lines of code per file.** Blank lines and comments don't count toward the limit.
- When a file approaches 40 lines, plan how it will be split before adding more.
- Splitting strategies: extract reusable behavior into a new script under `res://systems/`, extract data into a new `Resource`, or split a glue script by phase (e.g. `player_input.gd` + `player_motion.gd`).
...
- Validate every value entering the simulation from outside: save files, network responses, environment variables, user input fields, scraped data.
